def paralel_montecarlo_piqnn_schrodinger(j,max_epochs=10000, n_qubits=4, seed=128):
    ...
    def qnode(inputs, weights):
        ...
        return_list = []
        for i in range(n_qubits): return_list.append(qml.expval(qml.Z(i)))
        return return_list
    ...
    model.add(tf.keras.layers.Dense(units=n_qubits, activation=activation))
...
def paralel_montecarlo_piqnn_pendulum(j,max_epochs=10000, n_qubits=4, seed=128):
...
def paralel_montecarlo_piqnn_fraction(j,max_epochs=10000, n_qubits=4, seed=128):
